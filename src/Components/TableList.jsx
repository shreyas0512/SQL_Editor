...
function TableList() {
  const { role } = UserRole();
  const {savedQueries ,setSavedQueries}=CurrTable();
  ...
  return (
    <div className="h-1/2 flex flex-col justify-start sm:mb-2 mb-4 ">
      ...
      <div className="flex flex-col h-full justify-between">
        <div className="overflow-auto flex flex-col ">
          {tables.map((element, index) => {
            return (
              <button
                key={index}
                className={`text-center border-[1px] border-[#c8c6c6] mt-2 px-full py-1 mx-2 text-md rounded-md hover:bg-blue transition duration-50 ease-in-out hover:text-white ${
        ...
        <button
          className="text-blue font-semibold text-md border-[1.5px] py-0.5 border-blue mx-2 rounded-md mt-1 mb-2 hover:bg-blue hover:text-white transition duration-50 ease-in-out"
          onClick={addTable}
        >
          + Add Table
        </button>
